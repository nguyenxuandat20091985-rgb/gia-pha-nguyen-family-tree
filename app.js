...
function renderNode(id, depth){
  ...
  wrap.className='child-branch'+(depth===1?' main-branch':'');
  if(depth===1 && (p.branchLabel||BRANCH_LABEL[id])){
    const bl=document.createElement('div');
    bl.className='branch-label';
    bl.textContent=p.branchLabel||BRANCH_LABEL[id];
    wrap.appendChild(bl);
  }
  const couple=document.createElement('div');
  couple.className='couple';
  couple.appendChild(createCard(p, depth));
  (p.spouses||[]).forEach(sid=>{
    ...
    if(sp){
      ...
      couple.appendChild(createCard(sp, depth));
    }
  });
...
function createCard(p, depth){const card=document.createElement('div');card.className='person-card '+(p.gender||'male');if(p.isSide)card.classList.add('side-branch');if(p.isRoot||p.id===data.rootId)card.classList.add('root-card');card.dataset.id=p.id;card.style.cursor='pointer';if(p.isRoot||p.id===data.rootId){const b=document.createElement('div');b.className='root-badge';b.textContent='CỤ TỔ 6 ĐỜI';card.appendChild(b);}if(p.photo){const img=document.createElement('img');img.className='photo';img.src=p.photo;card.appendChild(img);}const name=document.createElement('div');name.className='name';name.textContent=p.name;card.appendChild(name);const parts=[];if(p.birthDate)parts.push(p.birthDate);if(p.deathDate)parts.push('– '+p.deathDate);if(parts.length){const dates=document.createElement('div');dates.className='dates';dates.textContent=parts.join(' ');card.appendChild(dates);}if(p.deathAnniversary){const g=document.createElement('div');g.className='gio-date';g.textContent='📅 Giỗ: '+p.deathAnniversary;card.appendChild(g);}card.addEventListener('click',function(e){e.stopPropagation();showContextMenu(e,p.id);});return card;}
function renderPersonList(){document.getElementById('treeView')?.classList.add('hidden');document.getElementById('listView')?.classList.remove('hidden');const q=(document.getElementById('searchTree')?.value||'').toLowerCase();const list=document.getElementById('personList');if(!list)return;let people=Object.values(data.people);if(q)people=people.filter(p=>(p.name||'').toLowerCase().includes(q));people.sort((a,b)=>(a.name||'').localeCompare(b.name||'','vi'));list.innerHTML=people.map(p=>{let meta=[p.birthDate,p.deathAnniversary?'Giỗ: '+p.deathAnniversary:''].filter(Boolean).join(' · ');if(p.id===data.rootId)meta='CỤ TỔ 6 ĐỜI'+(meta?' · '+meta:'');return '<div class="list-item" data-id="'+p.id+'"><div class="info"><div class="name">'+esc(p.name)+'</div><div class="meta">'+esc(meta)+'</div></div></div>';}).join('');list.querySelectorAll('.list-item').forEach(item=>{item.style.cursor='pointer';item.onclick=function(e){showContextMenu(e,item.dataset.id);};});}
...
document.getElementById('btnResetFinal')?.addEventListener('click',()=>{if((document.getElementById('resetTyped')?.value||'')!=='RESET'){alert('Gõ đúng RESET');return;}TREE_KEYS.forEach(k=>localStorage.removeItem(k));localStorage.removeItem('giaPhaSeedVersion');data={people:{},rootId:null};expandedNodes=new Set(['root']);seedIfEmpty();alert('Đã tải lại dữ liệu mẫu');showView('tree');});
