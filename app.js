...
let expandedBranches=new Set(['coc','lach','ngoc']);
const MAIN_BRANCHES=['coc','lach','ngoc'];
...
function seedIfEmpty(){if(typeof window.GIA_TREE_SEED==='function'){const next=window.GIA_TREE_SEED(data,saveTree);if(next)data=next;return;}if(Object.keys(data.people||{}).length)return;data.people.root={id:'root',name:'Nguyễn Văn Mương',gender:'male',birthDate:'',deathDate:'',deathAnniversary:'2/3',notes:'Cụ Tổ 6 đời',photo:null,children:[],spouses:[],sideBranches:[],isSide:false,isRoot:true,parentId:null,generation:6};data.rootId='root';saveTree();}
...
function collectByGen(branchId){
  const byGen={};
  ...
  while(queue.length){
    ...
    if(!p)continue;
    const g=p.generation!=null?p.generation:0;
    if(!byGen[g])byGen[g]=[];
    if(id!==branchId) byGen[g].push(p);
    (p.spouses||[]).forEach(sid=>{
      if(seen.has(sid))return;
      seen.add(sid);
      const sp=getPerson(sid);
      if(sp){
        const sg=sp.generation!=null?sp.generation:g;
        if(!byGen[sg])byGen[sg]=[];
        byGen[sg].push(sp);
      }
    });
...
function renderTree(){
  ...
  if(!hint){
    ...
    hint.textContent='📜 Giống sơ đồ tay · Vuốt ngang xem đủ 3 nhánh · Bấm ▲/▼ mở nhánh · Bấm tên để sửa';
    document.getElementById('treeView')?.insertBefore(hint, container);
  }
  container.innerHTML='';
  container.className='tree paper-layout';

  const rootWrap=document.createElement('div');
  rootWrap.className='paper-root';
  rootWrap.appendChild(createCard(getPerson(data.rootId)));
  container.appendChild(rootWrap);

  const line=document.createElement('div');
  line.className='paper-vline';
  container.appendChild(line);

  const cols=document.createElement('div');
  cols.className='paper-branches';

  MAIN_BRANCHES.forEach(bid=>{
    const head=getPerson(bid);
    if(!head)return;
    const col=document.createElement('div');
    col.className='paper-col'+(expandedBranches.has(bid)?' open':'');

    const label=document.createElement('button');
    label.type='button';
    label.className='paper-branch-btn';
    label.innerHTML='<span>'+(head.branchLabel||head.name)+'</span><span class="paper-branch-toggle">'+(expandedBranches.has(bid)?'▲':'▼')+'</span>';
    ...
    col.appendChild(createCard(head));

    if(expandedBranches.has(bid)){
      const byGen=collectByGen(bid);
      const gens=Object.keys(byGen).map(Number).sort((a,b)=>b-a);
      gens.forEach(g=>{
        const people=byGen[g];
        if(!people||!people.length)return;
        const genLabel=document.createElement('div');
        genLabel.className='paper-gen-label';
        genLabel.textContent='Đời '+g;
        col.appendChild(genLabel);
        const row=document.createElement('div');
        row.className='paper-gen-row';
        people.forEach(p=>{
...
function createCard(p){
  ...
  if(p.isRoot||p.id===data.rootId){
    const b=document.createElement('div');b.className='root-badge';b.textContent='CỤ TỔ 6 ĐỜI';card.appendChild(b);
  ...
  if(p.birthDate){const dates=document.createElement('div');dates.className='dates';dates.textContent=p.birthDate;card.appendChild(dates);}
  ...
  if(p.parentId&&data.people[p.parentId]&&!p.isRoot){
    const ph=document.createElement('div');
    ph.className='parent-hint';
    ph.textContent='← '+(data.people[p.parentId].name||'').replace('Nguyễn Văn ','N.V. ').replace('Nguyễn ','N. ');
    card.appendChild(ph);
  }
...
function savePerson(e){e.preventDefault();const name=(document.getElementById('fullName').value||'').trim();if(!name){alert('Nhập họ tên');return;}const editId=document.getElementById('personId').value;const parentId=document.getElementById('parentId').value;const relation=document.getElementById('relationType').value||'child';const fields={name,gender:document.getElementById('gender').value||'male',birthDate:document.getElementById('birthDate').value||'',deathDate:document.getElementById('deathDate').value||'',deathAnniversary:document.getElementById('deathAnniversary').value||'',notes:document.getElementById('notes').value||'',photo:photoBase64};if(editId&&data.people[editId]){Object.assign(data.people[editId],fields);}else{const id=uid();data.people[id]={id,...fields,children:[],spouses:[],sideBranches:[],isSide:relation==='side',isRoot:false,parentId:parentId||null};if(!data.rootId){data.rootId=id;data.people[id].isRoot=true;}else if(parentId&&data.people[parentId]){if(relation==='spouse'){if(!data.people[parentId].spouses.includes(id))data.people[parentId].spouses.push(id);if(!data.people[id].spouses.includes(parentId))data.people[id].spouses.push(parentId);}else if(relation==='side'){if(!data.people[parentId].sideBranches.includes(id))data.people[parentId].sideBranches.push(id);}else{data.people[id].parentId=parentId;if(!data.people[parentId].children.includes(id))data.people[parentId].children.push(id);}}}saveTree();closePersonModal();refreshTree();}
function deletePerson(id){const p=getPerson(id);if(!p)return;if(p.id===data.rootId){alert('Không xóa Cụ Tổ');return;}if(!confirm('Xóa '+p.name+'?'))return;Object.values(data.people).forEach(x=>{x.children=(x.children||[]).filter(c=>c!==id);x.spouses=(x.spouses||[]).filter(c=>c!==id);x.sideBranches=(x.sideBranches||[]).filter(c=>c!==id);});delete data.people[id];saveTree();refreshTree();}
...
document.getElementById('btnResetFinal')?.addEventListener('click',()=>{if((document.getElementById('resetTyped')?.value||'')!=='RESET'){alert('Gõ đúng RESET');return;}TREE_KEYS.forEach(k=>localStorage.removeItem(k));localStorage.removeItem('giaPhaSeedVersion');data={people:{},rootId:null};expandedBranches=new Set(['coc','lach','ngoc']);seedIfEmpty();alert('Đã tải lại cây theo sơ đồ tay');showView('tree');});
